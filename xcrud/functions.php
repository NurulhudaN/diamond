...
function publish_action($xcrud)
{
    if ($xcrud->get('primary'))
    {
        ...
        $query = 'UPDATE selling SET `status` = b\'1\' WHERE sid = ' . (int)$xcrud->get('primary');
        $db->query($query);
    }
}
function unpublish_action($xcrud)
{
    ...
    {
        $db = Xcrud_db::get_instance();
        $query = 'UPDATE selling SET `status` = b\'0\' WHERE sid = ' . (int)$xcrud->get('primary');
        $db->query($query);
    }
}
    ...
    */
    ...
    function get_amount($postdata,$primary, $xcrud)
    {
        $db = Xcrud_db::get_instance();
       // $pid = $db->insert_id();
        $purchase_rate = $postdata->get('rate');
        $purchase_weight = $postdata->get('weight');
        $purchase_amount = $purchase_rate * $purchase_weight;
        $sql = 'UPDATE `selling` SET `amount`=' . $purchase_amount . ' WHERE `sid`= ' . $db->escape($primary);
        $db = Xcrud_db::get_instance();
        $db->query($sql);
